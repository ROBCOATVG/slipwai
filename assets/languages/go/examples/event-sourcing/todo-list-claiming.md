...
// OwedSeat is one row of the todo list: a claim with no seat yet.
type OwedSeat struct {
	RegistrationID string
	ClaimedAt      string
}

// SeatsOwed hands out work no other runner is holding, for the life of the caller's transaction.
type SeatsOwed interface {
	ClaimBatch(ctx context.Context, limit int) ([]OwedSeat, error)
}
```
...
```go
// AllocateOwedSeats claims work, decides, and issues the command — through the same use case a person
// would have driven.
//
...
func AllocateOwedSeats(ctx context.Context, owed SeatsOwed, allocate AllocateSeatFunc, batch int) (int, error) {
	claims, err := owed.ClaimBatch(ctx, batch)
	if err != nil {
		return 0, fmt.Errorf("claim seats owed: %w", err)
	}
	issued := 0
	for _, claim := range claims {
		outcome, err := allocate(ctx, AllocateSeat{RegistrationID: claim.RegistrationID})
		if err != nil {
			return issued, fmt.Errorf("allocate seat for %s: %w", claim.RegistrationID, err)
		}
