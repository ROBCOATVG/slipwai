...
# Azure has no ambient region: every resource carries a `location`, so unlike AWS's `AWS_REGION` there is
# nothing for a provider to read out of the environment. `make bootstrap` passes `AZURE_LOCATION`, or what
# `az config get defaults.location` answers, and writes it to the forge so the pipeline passes the same one.
variable "location" {
  description = "The Azure region this project lives in, as `az account list-locations` names it: `uksouth`."
  type        = string
}

variable "repository" {
  description = "The repository whose main branch may deploy, as `owner/name`; what the federated credentials name. Read from the remote by `make bootstrap`."
  type        = string
}
...
variable "deploy_with" {
  description = "How the pipeline becomes the deploy identity: `oidc` (GitHub, no stored credential) or `access-key` (a forge without OIDC federation, such as Gitea). Decided by `make bootstrap` from the remote."
  type        = string
  default     = "oidc"
...
# `add-service`; the same file the service stack reads. Nothing here reads it — a container registry holds
# every repository without being told about any of them, which is the one place this stack is simpler than
# its AWS counterpart — and it is declared so that one tfvars file serves both stacks.
variable "services" {
