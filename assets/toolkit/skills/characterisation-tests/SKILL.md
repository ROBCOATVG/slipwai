...
| Adding a mocking framework (Mockito, Moq, gomock, `unittest.mock`) to pin behaviour | Pin at the widest real seam; where a collaborator must be replaced, a hand-written fake of its interface. Interaction assertions record shape, not behaviour, and break under the refactoring the pin exists to make safe |
