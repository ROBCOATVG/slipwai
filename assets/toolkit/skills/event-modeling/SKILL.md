...
repository keeps one cumulative global model in `docs/event-model/model.yaml`** (hyphen, one file), not a
per-workflow tree. Its schema is not free-form: read `docs/event-model/README.md` before a first entry, and
follow `skills/global-event-model/SKILL.md` for the recording procedure — that skill owns statuses, stream
...
|---|---|
| actors, major processes, external systems, which workflow to model first | `docs/event-model/model.yaml` slices at `status: proposed`, and the product specification the Spec Kit specify command writes |
...
in `project.json`; *Bounded contexts* in `docs/architecture.md`). A slice no
