...
- **Give "measurably slow" an instrument, or it never fires.** The condition is unobservable by default: nothing counts events folded per rehydration, so the trigger arrives as a production symptom long after the cheap fix has been built over. Write down the number of events the stream is expected to hold at its own end, keep a test that appends that many and asserts rehydration stays inside a budget, and let *that* failure be what sends you here — on a stream whose lifetime is already as short as the model can make it. Note that this bullet and *"you do not need a persisted read model, the streams are short"* are the same assumption spent twice: a project whose queries fold the log per request has no version ceiling on the read path at all, because a view folds across streams and no `expectedVersion` bounds it.
...
- **One stream for everything** — a single stream, or a stream shaped by convenience rather than by the invariant it protects. *Fix:* one stream per aggregate instance, chosen as the consistency boundary it creates (`event-store.md`).
