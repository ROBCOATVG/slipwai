...
- Adam Dymitruk, Event Modeling: https://eventmodeling.org/ + the [cheat sheet](https://eventmodeling.org/posts/event-modeling-cheatsheet/) — the timeline, the three swimlanes, the colour grammar, and the four patterns that `modelling-events.md` uses. The method this project models with; facilitation is the `event-modeling` skill.
- Martin Dilger, *Understanding Eventsourcing* — the nine-step workflow, vertical slices, and Given/When/Then per slice, as taught by the `event-modeling` skill.
- Alberto Brandolini, EventStorming: https://www.eventstorming.com/ + *Introducing EventStorming* (Leanpub) — background. A different workshop format for the same discovery; the source of the *pivotal events* idea kept in `modelling-events.md`. Not the method this project uses.
