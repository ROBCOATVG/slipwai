...
**A skill that is not in `skills/` may be missing on purpose.** This repository was given the part of the delivery catalogue it can use: a skill declares the capabilities it serves in its `SKILL.md` frontmatter, and one about something this project has no answer for — a backend for a frontend with no browser app, an OAuth guide with no identity provider — was not shipped. `docs/skills-and-commands.md` lists what is here and names what is not, with the capability each was waiting for. So before treating a gap as uncovered, read that page: a withheld skill is a sign the work you are about to do needs a capability this project has not got yet, and answering that — `slipwai add-frontend`, an identity provider — brings the skill with it. Do not install an ecosystem copy of one the factory withheld.
