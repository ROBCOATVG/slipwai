...
**In other languages** the module seam's counterpart is the mocking framework -- Mockito, Moq, gomock, `unittest.mock` -- and it has the same standing: last resort, scaffolding, never the recommendation. The examples here are TypeScript; the advice is not "use the mock". Where a collaborator must be replaced, write a fake that implements its real interface and holds state (see the `hexagonal-architecture` skill, `resources/testing-hex-arch.md`, "Fakes, Not Mocks").
...
| Adding a mocking framework because the examples here are Vitest | The examples are the language, not the advice: a hand-written fake of the real interface in the test tree; a framework mock is the same last resort as `vi.mock()` |
