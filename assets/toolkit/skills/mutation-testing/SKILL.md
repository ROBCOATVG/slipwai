...
In this repository, `/mutation` runs this gate: preconditions, `make mutation` over the domain, survivor triage ordered by production cost, and the kill loop below.

For attacking the behavior itself rather than measuring the tests over it, load `adversarial-testing`. It runs **first** at the same end-of-phase gate: mutation testing only mutates code that exists, so it is structurally blind to the branch nobody wrote, and it measures whatever suite exists when it runs — including the tests the adversarial pass just added.
...
END-OF-PHASE PR-READINESS GATE:
    ├─► Run the adversarial pass first (`adversarial-testing`), landing each break as a test
