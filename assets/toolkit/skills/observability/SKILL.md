...
**Adopting it in a language this project does not already wire** (see the next section for what is wired): the API package, the SDK package, and one instrumentation per boundary that matters. Auto-instrumentation gives HTTP, framework and database spans for free; manual attributes add the business meaning that makes traces queryable. `resources/node-patterns.md` has the Node particulars, including the one that bites — an instrumentation loaded after the modules it patches silently captures nothing.

---

## What This Project Already Has, and What It Does Not

Read this before instrumenting anything. Adding a second tracer provider beside the one that is already
registered is how a service ends up with two, each seeing half the spans.

**Wired**, in the service the transport axis answered for: a tracer provider with the W3C propagator,
started after the environment is checked and before anything binds; one span per request, continuing
whatever `traceparent` the caller sent; `trace_id` and `span_id` on every log record written inside one;
and, where this project records events, that trace and span as the event's `correlationId` and
`causationId` — so a transaction that began in another service keeps one correlation id across both.
**No exporter**: `OTEL_EXPORTER_OTLP_ENDPOINT` turns exporting on, and unset means spans are recorded,
given real ids and dropped. The service starts and the gate passes with nothing listening.

**Not wired, and deliberately:**

- **Metrics and logs as OTel signals.** Only traces. Logs go to stdout as structured records — what the
  `twelve-factor` skill asks for and what a shipper reads; an OTel log pipeline is a second route for the
  same lines.
- **Instrumentation beyond the transport**, and **sampling**: everything is recorded, which is right while
  nothing is exported and wrong at volume. Read the sampling ladder below before pointing at a collector.
- **A Collector.** There is none in `docker-compose.yml`. Adding one is a container and an endpoint.
- **Java.** Neither Java backend is instrumented, and it is one dependency and one property each: Quarkus
  takes the `quarkus-opentelemetry` extension with `quarkus.otel.exporter.otlp.endpoint` (and
  `quarkus.otel.sdk.disabled=true` while there is nowhere to send to); Spring Boot takes
  `spring-boot-starter-actuator` beside `micrometer-tracing-bridge-otel` and `opentelemetry-exporter-otlp`,
  with `management.otlp.tracing.endpoint`. Both send nowhere until an endpoint is set, which is the rule
  the other backends already follow. Do it in the project that needs it, and keep that default.
...
**Correlation:** every log record carries the active trace ID (W3C trace context), so logs join to traces and to the canonical event for free. This project's logger factory already does it; anything added there is added once rather than at call sites.
