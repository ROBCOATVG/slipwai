...
1. Run the quickest relevant tests in the same file or area and confirm they pass (or that the approved
   local preservation evidence still holds)
2. Present class-specific evidence: RED/GREEN for behavior change; preserved contract for pure refactor;
   passing behavior gate plus independent verification and pending mechanism gate/no net claim for a
   transition; or linked program/ledger, discharged obligations, both passing gates, and retired machinery
   for a terminal reduction
3. Do not require static analysis, affected suites, the full gate, or a mutation report for every increment
