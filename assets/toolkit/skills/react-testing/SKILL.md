...
**Before writing a component, read `docs/design.md` in this repository.** It records what this project's screens are made of — the design tokens, the layout shell, the element styles a component inherits without asking — so a new screen starts from the project's own baseline rather than from browser defaults, and a test asserting on a rendered page is asserting on the page this project actually has.
