...
- Use the quickest relevant test command scoped to the same file or area
...
- Keep those same local quick tests green after each small refactor; widen only after demo acceptance, immediately before the first implementation push
...
0. **ATTACK**: Run `adversarial-testing` first — an independent, read-only adversary tries to break the finished behavior from its contract. Each confirmed break becomes a failing test at the level that owns it, then a fix; each undefined behavior becomes a question, never an invented name. It precedes mutation deliberately: mutation measures the suite that exists when it runs, and this step changes that suite. It is also the only step here that can find a branch nobody wrote.
...
- [ ] The adversarial pass ran once against the finished behavior, its attempts are recorded and not only its findings, and every confirmed break landed as a test before its fix
