...
|---|---|---|---|
| `codegraph` | [CodeGraph](https://github.com/colbymchenry/codegraph), a local MCP code-knowledge graph, and indexes the tree | `.codegraph/` (ignored); the server named in each installed harness's project MCP file (committed, started through `npx`, so the connection travels with the checkout) | `scripts/codegraph`, the pinned CLI through `npx`, as the route every session has; Claude Code's `.mcp.json` entry with `alwaysLoad`, and hooks that refuse a symbol search of the source until the index was asked and sync it after each delegate; `check-codegraph` fails a corrupt index and one that stops describing the source; `/cruise` repairs and syncs the index before every iteration and counts its queries per delegate |
